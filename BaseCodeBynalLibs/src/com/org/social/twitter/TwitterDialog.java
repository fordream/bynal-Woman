...
public class TwitterDialog extends Dialog {
	...
	private void setUpTitle() {
		...
		Drawable icon = getContext().getResources().getDrawable(android.R.drawable.btn_star);
	...
	private class TwitterWebViewClient extends WebViewClient {
		...
		public boolean shouldOverrideUrlLoading(WebView view, String url) {
			Log.d(TAG, "Redirecting URL 1" + url);
			if (url.startsWith(TwitterUtils.STRING_CALLBACK)) {
				TwitterDialog.this.dismiss();
				mListener.onComplete(url);
				return true;
			} else if (url.startsWith(TwitterUtils.STRING_CALLBACK_DENIED)) {
				TwitterDialog.this.dismiss();
				return true;
