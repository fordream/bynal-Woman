...
public class ClientLoginAuthorizer implements Authorizer {
	...
	private Context ctx;
	private static final String LOG_TAG = ClientLoginAuthorizer.class.getSimpleName();
	...
	@Override
	public void fetchAuthToken(String accountName, Activity activity, AuthorizationListener<String> listener) {
		Log.d(LOG_TAG, "Getting " + YOUTUBE_AUTH_TOKEN_TYPE + " authToken for " + accountName);
		try {
	...
	public String getCLAuthToken(String accountName) throws IOException {
		...
		// String userName = "bynalytb@gmail.com"; // TODO
		// String password = "yt1212bynal"; // TODO
		String service = YOUTUBE_AUTH_TOKEN_TYPE;
		String source = ctx.getString(R.string.client_id);
		String loginData = String.format(template, encode(YouTubeManager.YOUTUBENAME), encode(YouTubeManager.YOUTUBE_PASSWORD), service, source);
		...
		if (responseCode != 200) {
			Log.d(LOG_TAG, "Got an error response : " + responseCode + " " + urlConnection.getResponseMessage());
	...
	public static class ClientLoginAuthorizerFactory implements AuthorizerFactory {
